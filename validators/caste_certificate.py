from utils.common import normalize_text, NAME_PATTERN, APPLICATION_NO_PATTERN
import re

def validate(form_data: dict, extracted_entities: dict, extracted_text: str) -> dict:
    """Validate Caste Certificate document."""
    required_fields = ["name", "father_name", "application_no", "caste"]
    ...
    for label in caste_labels:
        ...
        if label_index != -1:
            ...
            for line in lines:
                ...
                for caste_key, aliases in caste_mapping.items():
                    if any(alias in line for alias in aliases):
                        validation_result["caste"]["extracted_value"] = caste_key
                        form_caste = normalize_text(form_data["caste"])
                        if form_caste in aliases or form_caste == caste_key:
                            validation_result["caste"]["is_valid"] = True
                        else:
                            validation_result["caste"]["error"] = "Caste does not match the extracted value."
                        break
                if validation_result["caste"]["extracted_value"]:
                    break
            break
    if not validation_result["caste"]["extracted_value"]:
        validation_result["caste"]["error"] = "Caste not found in extracted data."
